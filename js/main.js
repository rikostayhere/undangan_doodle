...
(function () {
  ...
  /* ------------------------------------------------------------------ */
  /* Helper aman untuk eksekusi fungsi tanpa menghentikan script lain   */
  /* ------------------------------------------------------------------ */
  function safeRun(desc, fn) {
    try {
      fn();
    } catch (err) {
      console.warn(`[Undangan] Peringatan di bagian ${desc}:`, err);
    }
  }

  /* Data pernikahan dengan fallback aman jika data.js ada kendala */
  const wedding = (typeof weddingData !== "undefined" && weddingData) ? weddingData : (window.weddingData || {});

  /* ------------------------------------------------------------------ */
  /* 1. Cover open interaction & Core UI setup                          */
  /* Diinisialisasi di paling awal agar tombol BUKA UNDANGAN selalu      */
  ...
  function openCover() {
    ...
    // Sembunyikan cover display setelah transisi zoom out selesai agar tidak menghalangi scroll & klik
  ...
  safeRun("Guest Name", () => {
    ...
    if (guestName) {
      ...
      if (guestEl) guestEl.textContent = decodeURIComponent(guestName);
    }
  });

  /* ------------------------------------------------------------------ */
  /* 3. Isi doodle-doodle statis                                         */
  /* ------------------------------------------------------------------ */
  safeRun("Static Doodles", () => {
    if (typeof doodles === "undefined") return;
    ...
    // Ambient floating doodles scattered on hero
  ...
  /* ------------------------------------------------------------------ */
  /* 4. Render data pernikahan ke DOM                                    */
  /* ------------------------------------------------------------------ */
  ...
  safeRun("Cover Photo", () => {
    if (wedding.coverPhoto) {
      ...
      if (coverPhotoEl && wedding.coverPhoto.src) {
        coverPhotoEl.src = wedding.coverPhoto.src;
      }
      if (coverCaptionEl && wedding.coverPhoto.caption) {
        coverCaptionEl.textContent = wedding.coverPhoto.caption;
      }
    }
  });
  ...
  safeRun("Couple Data", () => {
    if (wedding.groom) {
      ...
      const gParents = document.getElementById("groom-parents");
      if (gPhoto && wedding.groom.photo) gPhoto.src = wedding.groom.photo;
      if (gName) gName.textContent = wedding.groom.fullName || "";
      if (gParents) gParents.textContent = wedding.groom.parents || "";
    }

    if (wedding.bride) {
      const bPhoto = document.getElementById("bride-photo");
      const bName = document.getElementById("bride-name");
      const bParents = document.getElementById("bride-parents");
      if (bPhoto && wedding.bride.photo) bPhoto.src = wedding.bride.photo;
      if (bName) bName.textContent = wedding.bride.fullName || "";
      if (bParents) bParents.textContent = wedding.bride.parents || "";
    }

    const hashtagEl = document.getElementById("closing-hashtag");
    if (hashtagEl && wedding.hashtag) {
      hashtagEl.textContent = wedding.hashtag;
    }

    const bgMusic = document.getElementById("bg-music");
    if (bgMusic && wedding.music && wedding.music.src) {
      bgMusic.src = wedding.music.src;
    }
  });
  ...
  safeRun("Event Cards", () => {
    ...
    if (!eventCardsEl) return;
    const events = [wedding.akad, wedding.reception].filter(Boolean);
    events.forEach((ev) => {
      const card = document.createElement("div");
      card.className = "event-card reveal-up";
      const calIcon = typeof doodle === "function" ? doodle("calendar") : "";
      const clockIcon = typeof doodle === "function" ? doodle("clock") : "";
      const pinIcon = typeof doodle === "function" ? doodle("pin") : "";
      card.innerHTML = `
        <p class="event-label">${ev.label || ""}</p>
        <div class="event-row">${calIcon}<span>${ev.date || ""}</span></div>
        <div class="event-row">${clockIcon}<span>${ev.time || ""}</span></div>
        <p class="event-venue">${ev.venue || ""}</p>
        <div class="event-row">${pinIcon}<span>${ev.address || ""}</span></div>
        ${ev.mapsUrl ? `<a class="btn-outline" href="${ev.mapsUrl}" target="_blank" rel="noopener">LIHAT LOKASI</a>` : ""}
      `;
  ...
  safeRun("Story Timeline", () => {
    const timelineEl = document.getElementById("timeline-list");
    if (!timelineEl || !Array.isArray(wedding.story)) return;
    const markIcons = ["leaf", "bird", "flowerSmall", "ring", "heartSmall"];
    wedding.story.forEach((item, i) => {
        ...
        ? `
          <div class="story-polaroid-wrap">
            <div class="polaroid story-polaroid ${tiltClass}">
              ...
              <img src="${item.image}" alt="${item.caption || item.title || "Story"}" loading="lazy" />
      ...
      const markIcon = typeof doodle === "function" ? doodle(markIcons[i % markIcons.length]) : "";
      row.innerHTML = `
        <span class="timeline-mark">${markIcon}</span>
  ...
  // Gallery scrapbook
  safeRun("Gallery Scrapbook", () => {
    const scrapbookEl = document.getElementById("scrapbook-list");
    if (!scrapbookEl || !Array.isArray(wedding.gallery)) return;
    wedding.gallery.forEach((photo) => {
  ...
  safeRun("Gift Cards", () => {
    ...
    if (!giftCardsEl || !Array.isArray(wedding.gifts)) return;
    wedding.gifts.forEach((g) => {
      const card = document.createElement("div");
  ...
  /* ------------------------------------------------------------------ */
  safeRun("Countdown", () => {
    if (!wedding.date || !wedding.date.iso) return;
